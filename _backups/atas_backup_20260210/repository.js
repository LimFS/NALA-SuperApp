        ...
        db.get("SELECT count(*) as count FROM course_offerings", (err, row) => {
            if (!err && row.count === 0) {
                ...
                db.run(`INSERT INTO course_offerings (course_code, course_name, academic_year, semester, is_active, icon_url) 
                        VALUES ('MH1810', 'Mathematics 2', 'AY2025', 'Semester 2', 1, '/mh1810/mh1810_math.png')`);
            }
        });
        ...
        db.get("SELECT count(*) as count FROM questions", (err, row) => {
            if (!err && row.count === 0) {
                ...
                const questions = [
                    ...
                    { id: 'q3', text: 'Explain why the function shown in the graph is discontinuous at x=0.', type: 'text', diff: 2, set: 1, media: { type: 'image', url: '/mh1810/discontinuity_jump.png', alt: 'Graph showing jump discontinuity' } },

                    // Level 3: Apply (1 Question + Image)
                    { id: 'q4', text: 'Calculate the average velocity between t=0 and t=5s based on the displacement graph.', type: 'text', diff: 3, set: 1, media: { type: 'image', url: '/mh1810/velocity_graph.png', alt: 'Displacement vs Time Graph' } },
