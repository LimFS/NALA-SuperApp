  ...
  <div v-else class="h-full flex flex-col bg-gray-50 text-gray-900 font-sans">
    ...
    <div class="flex-none bg-white border-b border-gray-200 flex flex-col md:flex-row items-center justify-between px-4 py-3 md:px-6 md:py-0 shadow-sm gap-3 md:gap-0 sticky top-0 z-10">
      ...
      <div class="flex items-center gap-4 w-full md:w-auto justify-between md:justify-end">
        ...
        <button @click="handleLogout" class="p-2 text-gray-400 hover:text-red-600 transition-colors uppercase text-xs font-bold tracking-wider" title="Logout">
            Logout
        </button>
...
const loadUserContext = async () => {
    try {
        ...
        // Fallback for Dev/Legacy (if no token but user param exists)
        if (!token && legacyUser) {
             activeUserId.value = legacyUser;
             // Should verify this against DB if possible, but we trust the param in legacy mode
        }
        ...
        if (activeUserId.value) {
            ...
            if (setsRes.ok) {
                ...
                if (fetchedSets.value.length > 0) {
                    ...
                    if (!exists) currentSetId.value = fetchedSets.value[0].set_id;
                }
            }
