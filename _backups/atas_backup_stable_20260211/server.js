...
app.post('/api/grade', aiLimiter, async (req, res) => {
    try {
        ...
        if (studentAnswer && questionId) {
            ...
            // 2. Construct Prompt (Simplified for ATAS - duplicate logic from ai-grader should be shared lib)
            // Hardcoded template for now or fetch from Config
            const promptText = `You are an AI Tutor. Analyze the answer.
Question: ${question ? question.question_text : "Unknown"}
Correct Answer: ${question ? JSON.stringify(question.answerKey) : "Unknown"}
...
app.post('/api/auth/exchange-token', (req, res) => {
    ...
    console.log("[ATAS] Received Token Exchange Request", token);

    if (!token) return res.status(400).json({ error: "No Token" });

    const data = decryptToken(token);
    if (!data) {
        console.error("[ATAS] Token Decryption Failed. Check Shared Secret.");
        return res.status(403).json({ error: "Invalid Token" });
    }

    console.log("[ATAS] Decrypted Token Data:", data);

    // Optional: Check Timestamp (Experation)
    const timeDiff = Date.now() - data.timestamp;
    console.log("[ATAS] Token Age (ms):", timeDiff);

    if (timeDiff > 60000) { // 1 Minute Validity
        console.error("[ATAS] Token Expired");
