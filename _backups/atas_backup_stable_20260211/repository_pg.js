...
const getQuestions = async (courseCode, academicYear, semester, setId = null, includeHidden = true) => {
    let query = `
        SELECT DISTINCT ON (q.id) 
               q.id, qv.uuid as version_uuid, qv.*, qs.name as set_name, qs.sequence_order,
...
const getProgress = async (userId, courseCode, academicYear, semester) => {
    const { rows } = await pool.query(
        "SELECT * FROM student_progress WHERE user_id = $1 AND course_code = $2 AND academic_year = $3 AND semester = $4",
...
const getQuestionStats = async (courseCode, academicYear, semester) => {
    const sql = `
            ...
            GROUP BY q.id, qs.name, qv.question_text, qv.context
