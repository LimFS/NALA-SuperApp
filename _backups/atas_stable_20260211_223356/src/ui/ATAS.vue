...
const loadSetState = (setId) => {
    ...
    if (fullProgressCache.value.sets && fullProgressCache.value.sets[setId]) {
        ...
        currentQuestionIndex.value = s.currentQuestionIndex ?? 0;
...
const loadUserContext = async () => {
    try {
        ...
        let identifiedEmail = legacyUser; // Fallback? Or Block? Secure mode: Prefer Token.
        ...
        if (activeUserId.value) {
            ...
            if (configRes.ok) {
                ...
                if (data.courseName) fetchedCourseName.value = data.courseName; // Self-heal name 
            }
            
            const setsRes = await fetch(`${API_BASE}/sets/${props.courseCode}?userId=${activeUserId.value}&academicYear=${ay}&semester=${sem}`);
            if (setsRes.ok) {
                const setsData = await setsRes.json();
                console.log("ATA: DEBUG RAW SETS DATA:", setsData); // Check if sequence_order exists
                
                // SORT IMMEDIATELY to prevent default ID=1 from taking over
                ...
                console.log("ATA: DEBUG SORTED SETS:", fetchedSets.value.map(s => ({ id: s.set_id, seq: s.sequence_order })));

                if (fetchedSets.value.length > 0) {
                    if (!currentSetId.value) {
                         // Now this will correctly pick Set 2 (Sequence 1)
