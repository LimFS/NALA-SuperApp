...
const getSets = async (courseCode, academicYear, semester, includeHidden = true) => {
    let query = "SELECT * FROM question_sets WHERE course_code = $1 AND academic_year = $2 AND semester = $3 AND deleted_at IS NULL";
    if (!includeHidden) {
...
const getQuestions = async (courseCode, academicYear, semester, setId = null, includeHidden = true) => {
    ...
    const safeParse = (str, context) => {
        if (!str) return null;
        if (typeof str === 'object') return str; // Already parsed (e.g. jsonb)
        try {
            return JSON.parse(str);
        } catch (e) {
            console.error(`[PG] JSON Parse Error in ${context}:`, e.message, "Value:", str);
            return null;
        }
    };
    ...
    const questions = rows.map(q => ({
        ...q,
        options: safeParse(q.options, `options (ID: ${q.id})`),
        answerKey: safeParse(q.answer_key, `answerKey (ID: ${q.id})`) || [],
        media: safeParse(q.media, `media (ID: ${q.id})`),
...
const upsertSet = async (courseCode, academicYear, semester, set, userEmail) => {
    ...
    return pool.query(
        ...
        [courseCode, academicYear, semester, setId, name, isVisible === undefined ? true : isVisible, sequenceOrder, difficultyId, difficultyName, encryptedEmail] // default is_visible true if undefined
    );
};
