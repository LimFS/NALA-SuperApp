...
export const gradeAnswer = async (questionText, studentAnswer, rubric, context, hint, correctAnswer) => {
    ...
    // --- FALLBACK LOGIC ---
    const useFallback = () => {
        console.warn("Using Fallback Grading (String Match).");
        if (!studentAnswer) return { score: 0, feedback: "No answer provided.", isCorrect: false };
        ...
        if (normCorrect && normStudent.includes(normCorrect)) {
            ...
            isCorrect = true;
        } else if (normStudent.length > 5) {
            score = 2; // Participation
            feedback = "Participation points awarded (AI Service Unavailable).";
        } else {
            feedback = "Answer too short / Incorrect (Fallback Mode).";
        }

        return { score, feedback, isCorrect };
    };

    if (!apiKey) return useFallback();
