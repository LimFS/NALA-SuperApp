...
console.log(`[ATAS Server] Using Repository: Postgres`);
...
app.get('/health', (req, res) => {
    res.json({ status: 'ATAS Agent Operational', service: 'mh1810', timestamp: new Date().toISOString() });
});
...
Respond STRICTLY in JSON format:
{
  ...
  "score": number (0-10)
        ...
        fs.appendFileSync(join(__dirname, 'logs', 'debug_error.log'), logMsg); // Persistent Log
        res.status(500).json({ error: err.message });
    }
});
